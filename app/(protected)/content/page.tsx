...
import { Plus, Bookmark, Trash2, ExternalLink, X, Utensils, MapPin, Mountain, Shirt, LayoutGrid } from 'lucide-react'
import Image from 'next/image'

interface ContentItem {
  id: string; url: string; notes: string | null
  thumbnail_url: string | null; category: string; created_at: string
}
...
export default function ContentPage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const { error: err } = await supabase.from('content_items').insert({
      user_id: user.id, url: validUrl, notes: notes.trim() || null, thumbnail_url: thumbnail, category,
    })
    setSaving(false)
    if (err) { setError(err.message); return }
    setUrl(''); setNotes(''); setCategory('food'); setShowAdd(false); load()
  }
  ...
  return (
    <div className="min-h-screen pt-6 page-enter">
      ...
      {showAdd && (
        <div className="mx-5 card rounded-2xl p-4 mb-4">
          <form onSubmit={handleSave} className="flex flex-col gap-3">
            <input className="input text-sm" placeholder="Paste URL…" value={url} onChange={e => setUrl(e.target.value)} type="url" autoFocus />
      ...
      {/* Category tabs */}
      <div className="flex gap-2 overflow-x-auto pb-2 px-5 mb-4">
        {CATEGORIES.map(cat => {
          const CatIcon = cat.icon
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <p className="text-xs font-medium truncate" style={{ color: catInfo.color }}>{getDomain(item.url)}</p>
                    ...
                    {item.notes && <p className="text-sm font-medium line-clamp-2 leading-tight">{item.notes}</p>}
